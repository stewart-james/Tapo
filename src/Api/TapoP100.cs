...
public class TapoP100 : ITapoP100
{
  ...
  protected Uri AppTokenUri => new Uri(BaseUri, $"app?token={Token}");
    ...
    public async Task TurnOff()
  {
    ...
    var response = await SecurePassThrough<SetDeviceInfoRequest, SetDeviceInfoResponse>(
        uri,
        new SetDeviceInfoRequest(false));
  }
  ...
  private async Task Handshake()
  {
    ...
    using(var rsa = RSA.Create(1024))
    {
      ...
      var key = keyParts.AsSpan().Slice(0, 16).ToArray();
      var iv = keyParts.AsSpan().Slice(16, 16).ToArray();

      var aes = Aes.Create();
      this._encryptor = aes.CreateEncryptor(key, iv);
      this._decryptor = aes.CreateDecryptor(key, iv);
    }
  }
  ...
  protected async Task<TResponse> SecurePassThrough<TRequest, TResponse>(Uri uri, TRequest request)
  {
    ...
    {
      throw new Exception($"Failed to parse {decryptedContent} into type {typeof(TRequest)}");
    }
  ...
  private sealed record HandshakeRequest : Request
  {
    ...
    {
      this.Params = new Payload(publicKey);
    }
  }
